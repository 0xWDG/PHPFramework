...
public extension String {
	...
	func between(_ left: String, _ right: String) -> String? {
        if let leftRange = range(of: left) {
            if let rightRange = range(of: right, options: .backwards) {
                if leftRange != rightRange && leftRange.upperBound != rightRange.lowerBound {
	...
	func camelize() -> String {
		...
		if source.contains(" ") {
			...
			var first = (source as NSString).lowercased
            first = String(first.suffix(from: source.index(source.startIndex, offsetBy: 1)))
	...
	func stripPunctuation() -> String {
		return components(separatedBy: NSCharacterSet.punctuationCharacters)
			.joined(separator: "")
